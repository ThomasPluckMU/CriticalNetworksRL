...
class PPOAgent(BaseAtariAgent):
    ...
    def __init__(self, config: Dict, action_space: int):
        ...
        # build conv backbone
        self.conv1, self.conv2, self.conv3 = _init_conv_layers(self.frame_stack, self.device)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        z1 = self.conv1(x); a1 = self.activation(z1)
        z2 = self.conv2(a1); a2 = self.activation(z2)
        z3 = self.conv3(a2); a3 = self.activation(z3)
        ...
        z4 = self._fc(flat); a4 = self.activation(z4)
    ...
    def act(self, state: torch.Tensor) -> int:
            ...
            if random.random() < self.config.get('epsilon', 0.0):
