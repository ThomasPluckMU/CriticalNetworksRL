...
for file in agents_dir.glob("*.py"):
    ...
    for name, cls in vars(module).items():
        if isinstance(cls, type) and issubclass(cls, BaseTrainer) and cls != BaseTrainer:
            AGENT_REGISTRY[name] = cls

def get_trainer_class(name: str) -> Type[BaseTrainer]:
    """Get trainer class by name from registry"""
    if name not in AGENT_REGISTRY:
        raise ValueError(f"Unknown trainer: {name}. Available trainer: {list(AGENT_REGISTRY.keys())}")
