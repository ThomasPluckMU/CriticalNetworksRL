...
class SARSALogic(TrainingLogic):
    ...
    def _update_network(
        self, agent, memory, current_next_state=None, current_next_action=None
    ):
        """
        ...
        next_action_batch = torch.LongTensor(
            np.array(
                [agent.act(torch.FloatTensor(s).to(agent.device)) for s in next_states]
            )
        ).to(agent.device)
        ...
        with torch.no_grad():
            ...
            next_q_values = next_q_values.gather(
                1, next_action_batch.unsqueeze(1)
            ).squeeze()
