...
class TrainingLogic(ABC):
    ...
    @abstractmethod
    def run_episode(
        self, env: Any, agent: Any, memory: Any, episode_idx: int
    ) -> Tuple[float, Dict]:
...
for file in logic_dir.glob("*.py"):
    ...
    for name, cls in vars(module).items():
        if (
            isinstance(cls, type)
            and issubclass(cls, TrainingLogic)
            and cls != TrainingLogic
        ):
            LOGIC_REGISTRY[name] = cls


def get_logic_class(name: str) -> Type[TrainingLogic]:
    """Get logic class by name from registry"""
    if name not in LOGIC_REGISTRY:
        raise ValueError(
            f"Unknown logic: {name}. Available logic: {list(LOGIC_REGISTRY.keys())}"
        )
    return LOGIC_REGISTRY[name]
