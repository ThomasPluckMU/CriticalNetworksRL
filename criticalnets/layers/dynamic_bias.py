...
class DynamicBiasBase(nn.Module):
    ...
    def _init_bias(self, batch_size):
        ...
        return torch.zeros(batch_size, self.hidden_size, device=self.device_param.device)
...
class DynamicBiasNN(DynamicBiasBase):
    def __init__(self, input_size, hidden_size, velocity_init=0.1, bias_decay=1.0):
        ...
        self.bias = nn.Parameter(torch.Tensor(hidden_size))  # This is a nn.Parameter now, not a buffer
        self.selu = nn.SELU()
...
def debug_gradients():
    ...
    try:
        ...
        if model.bias.grad is not None:
            print(f"Bias gradient norm after multiple passes: {model.bias.grad.norm().item():.6f}")
        else:
