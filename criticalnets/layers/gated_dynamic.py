...
class GatedDynamicBiasNN(DynamicBiasNN):
    ...
    def forward(self, x):
        ...
        gated_velocity = self.gelu(
            torch.matmul(x, self.velocity_weight.t()) + self.velocity_bias
        )
...
def debug_gradients():
    ...
    with torch.no_grad():
        ...
        print(
            f"Are both mechanisms active? {'Yes' if inherited_effect.norm().item() > 0 and gated_effect.norm().item() > 0 else 'No'}"
        )
